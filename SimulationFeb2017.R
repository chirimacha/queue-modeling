...
info <- c(avg.no.in.system / time, mean(customer.total.time[warmup.no.cust:target.no.customers]), mean(redisVect))
print(info)

# cumulative average 
cumAvgCustTime <- cumsum(customer.total.time) / seq_along(customer.total.time)
plot(cumAvgCustTime, type = "l", main = paste("Lambda = 37 & Mu = 40"), xlab = "No. of Customers")
